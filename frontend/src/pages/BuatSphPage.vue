            ...
            >
              ...
              <template v-if="isWeighted(activeItem)">
                ...
                <div class="flex items-end justify-end md:col-span-1">
                  <button type="button" class="rounded px-2 py-1.5 text-xs font-medium text-red-600 hover:bg-red-50" @click="removeSubItem(activeItem, sIdx)">✕</button>
                </div>
              </template>
              <template v-else>
                <div class="md:col-span-1">
...
import type { SphSaveInput } from '../types/collaboration'
...
async function save() {
  if (isCollabMode.value && !collabStore.isHost) return
  saving.value = true
  saveError.value = ''
  try {
    const payload = {
...
onMounted(async () => {
  ...
  if (isCollabMode.value) {
    EventsOn('collab:sync', onCollabSync)
  }
})
