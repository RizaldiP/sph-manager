<template>
  <AppModal :model-value="modelValue" title="Gabung via IP" @update:model-value="$emit('update:modelValue', $event)">
    ...
    <div class="space-y-4">
      <div>
        <label class="mb-1 block text-[13px] font-medium text-slate-600">Host IP <span class="text-red-500">*</span></label>
        <input v-model="hostIP" type="text" placeholder="192.168.1.100" class="w-full rounded-lg border border-slate-200 px-3 py-2 text-[13px] font-mono outline-none focus:border-brand-400 focus:ring-2 focus:ring-brand-100" />
      </div>
      <div>
        <label class="mb-1 block text-[13px] font-medium text-slate-600">Port <span class="text-red-500">*</span></label>
        <input v-model.number="port" type="number" min="1024" max="65535" class="w-full rounded-lg border border-slate-200 px-3 py-2 text-[13px] font-mono outline-none focus:border-brand-400 focus:ring-2 focus:ring-brand-100" />
      </div>
      <div>
      ...
      <div>
        <label class="mb-1 block text-[13px] font-medium text-slate-600">Room Code</label>
        <input v-model="roomCode" type="text" maxlength="10" placeholder="kode room (opsional)" class="w-full rounded-lg border border-slate-200 px-3 py-2 text-[13px] font-mono outline-none focus:border-brand-400 focus:ring-2 focus:ring-brand-100" />
      </div>
    </div>
...
const canJoin = computed(() => hostIP.value.trim() && displayName.value.trim() && port.value > 0)
...
watch(
  ...
  (open) => {
    ...
    port.value = collabStore.defaults?.port ?? 48765
    hostIP.value = ''
    accessCode.value = ''
    roomCode.value = ''
    pageError.value = ''
